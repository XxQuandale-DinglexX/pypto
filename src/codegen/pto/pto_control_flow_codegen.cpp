  ...
  if (op->return_vars_.empty()) {
    ...
    auto emit_branch = [&](const StmtPtr& body, const char* branch_name) {
      ...
      for (size_t i = 0; i < op->return_vars_.size(); ++i) {
        ...
        // acc→acc tmov is not supported on A5; skip when both tiles are in
        // the accumulator (they share the same physical buffer).
        if (src_type.find("loc=acc") != std::string::npos &&
            tile_return_types[i].find("loc=acc") != std::string::npos) {
          continue;
        }
